...
export const GET = withErrorHandler(async (req) => {
  ...
  return apiPaginated(
    posts.map((p) => ({
      id: p.id,
      slug: p.slug,
      title: zhCn.title(lang, p.title, p.title_zh),
      excerpt: zhCn.excerpt(lang, p.excerpt, p.excerpt_zh),
      image: p.image,
      content: includeBody ? zhCn.content(lang, p.content, p.content_zh) : undefined,
