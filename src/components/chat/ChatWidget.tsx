...
/**
 * Support chat widget. Two layers:
 *  - AI mode: OpenRouter-backed assistant with one-tap FAQ chips.
 ...
 * A gentle auto-open notice invites interaction once per visitor per day.
 */
  ...
  useEffect(() => {
    try {
      ...
      if (Date.now() - last > 1000 * 60 * 60 * 24) {
        ...
        const hide = setTimeout(() => setNoticeOpen(false), 12_000)
  ...
  return (
    <>
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="glass-panel !rounded-2xl fixed bottom-24 left-4 right-4 z-50 flex max-h-[min(560px,calc(100dvh-7rem))] flex-col overflow-hidden shadow-2xl sm:left-auto sm:right-6 sm:w-[380px]"
            role="dialog"
            aria-modal="false"
            aria-label={c.title}
          >
            ...
            <div className="flex items-center justify-between gap-2 border-b border-border/60 bg-bg-base/40 px-4 py-3">
              <div className="flex min-w-0 items-center gap-2">
                <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg border border-signal/25 bg-signal/15">
                  {mode === 'human-chat' ? <Headset size={14} className="text-signal" aria-hidden="true" /> : <Bot size={14} className="text-signal" aria-hidden="true" />}
                </div>
            ...
            {mode === 'human-form' ? (
              ...
              <>
                {/* messages */}
                <div ref={scrollRef} className="min-h-[280px] flex-1 space-y-4 overflow-y-auto p-4">
                  {allMessages.map((msg) => (
                    msg.role === 'system' ? (
                      ...
                      <div key={msg.id} className={cn('flex gap-2.5', msg.role === 'user' ? 'flex-row-reverse' : 'flex-row')}>
                        <div className={cn('mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full border', msg.role === 'user' ? 'border-border bg-bg-elevated' : 'border-signal/25 bg-signal/15')}>
                        ...
                        <div className={cn(
                          'max-w-[80%] min-w-0 break-words rounded-xl px-3 py-2 text-sm leading-relaxed',
                          msg.role === 'user'
                            ? 'rounded-br-sm border border-border bg-bg-elevated text-text-primary'
                  ...
                  {isLoading && (
                    <div className="flex gap-2.5" role="status" aria-live="polite">
                      ...
                      <div className="rounded-bl-sm rounded-xl border border-border/60 bg-bg-base/60 px-3 py-2">
                        <div className="flex gap-1" aria-hidden="true">
                          <span className="h-1.5 w-1.5 rounded-full bg-signal/60 motion-safe:animate-pulse" />
                          <span className="h-1.5 w-1.5 rounded-full bg-signal/60 motion-safe:animate-pulse [animation-delay:150ms]" />
                          <span className="h-1.5 w-1.5 rounded-full bg-signal/60 motion-safe:animate-pulse [animation-delay:300ms]" />
                        </div>
                      </div>
                    </div>
                  )}
                ...
                {/* FAQ chips */}
                {showFaqChips && (
                  <div className="border-t border-border/60 bg-bg-base/30 px-3 pb-1 pt-2.5">
                    <p className="mb-1.5 flex items-center gap-1 font-mono text-[9px] uppercase tracking-[0.16em] text-text-muted">
                    ...
                    <div className="flex flex-wrap gap-1.5 pb-1.5">
                      {faqs.map((f) => (
                        <button
                          key={f.q}
                          type="button"
                          onClick={() => { setMessages(prev => [...prev, { id: `u-${Date.now()}`, role: 'user', content: f.q }, { id: `a-${f.q}`, role: 'assistant', content: f.a }]) }}
                          className="rounded-full border border-signal/25 bg-signal/5 px-3 py-1.5 text-xs text-signal transition-colors hover:bg-signal/15"
                        >
                          {f.q}
                        </button>
                      ))}
      ...
      <AnimatePresence>
        {noticeOpen && !isOpen && (
          <motion.div
            initial={reduce ? { opacity: 0 } : { opacity: 0, x: 12, scale: 0.95 }}
            animate={{ opacity: 1, x: 0, scale: 1 }}
            exit={reduce ? { opacity: 0 } : { opacity: 0, x: 8, scale: 0.97 }}
            transition={{ duration: 0.25, ease: EASE_OUT, delay: 0.1 }}
            className="glass-panel fixed bottom-24 right-4 z-50 max-w-[280px] rounded-2xl p-4 pr-8 shadow-2xl sm:right-6"
            role="status"
          >
            <button
              type="button"
            ...
            <p className="pr-2 text-sm leading-relaxed text-text-secondary">{c.notice}</p>
            <button
              type="button"
              onClick={openChat}
              className="glass-cta mt-3 inline-flex items-center gap-1.5 !px-3 !py-1.5 !text-xs"
            >
      ...
      <button
        ...
        className={cn(
          ...
          isOpen ? 'glass-panel !rounded-full text-text-secondary' : 'border border-signal/40 bg-signal text-bg-base'
        )}
